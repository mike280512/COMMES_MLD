...
hp = RevWaterSourceHPModel('hp',N,bigM,cop,eer);
hp.add_on_off('hp_y_e',0.1,20);
...
qPwr = 2.5;
qConv = 1/(1000*4*4.18);
pump = RevNetworkPumpModel('pump',N,bigM,onPwr,qPwr,qConv);
...
%% initialise hub
hub = Aggregation('hub',N,eBal,hBal,ed,hd,cd,tx,hp,pump);

%connect ports
hub.connect('tx_y_out','eBal_y_snk1',1);
hub.connect('eBal_y_src1','hp_y_e',1);
hub.connect('eBal_y_src2','ed_y',1);
hub.connect('pump_y_out','hp_y_in',1);
hub.connect('hp_y_out','hBal_y_snk1',1);
hub.connect('cd_y','hBal_y_snk2',1);
...
cd_f = datatable{N+1:2*N,'HeatingGaskWHourly'}; %use heating data for ease

classdef PDemandModel < Component
    
    % PDemandModel Static MLD system model to allow input of nominal energy
    % demand which is fulfilled over a given period. Assumed to be a
    ...
    methods
        ...
        function self = PDemandModel(name,N,bigM,Nn)
        ...
        function m = get_m(self)
            ...
            varName_m_procMin = [self.name,'_m_procMin'];
            varName_m_procMax = [self.name,'_m_procMax'];
            ...
            self.vars.(matlab.lang.makeValidName(varName_m_procMin)) = sdpvar(1,self.Nn);
            self.vars.(matlab.lang.makeValidName(varName_m_procMax)) = sdpvar(1,self.Nn);
            
            m = [m;varName_m_E;varName_m_proc;varName_m_comp;varName_m_procMin;varName_m_procMax];
        end
        
        function C = get_C(self)
            C = self.C;
            %construct constraints for PDemandModel
            ...
            var_m_procMin = self.vars.(matlab.lang.makeValidName(varName_m_procMin));
            var_m_procMax = self.vars.(matlab.lang.makeValidName(varName_m_procMax));
            ...
            C = C + [(var_m_procMin <= ones(1,self.N)*var_d_proc <= var_m_procMax):strcat(varName_d_proc,'_limits')];
